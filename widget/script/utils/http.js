define(function(require, exports, module) {
    ...
    function Http() {
        this._opts = {
            ...
            // MD5加密串
            // token: '',
            company_id: (UserInfo && UserInfo.company_id) ? UserInfo.company_id : '',
        };
        this.isLock = false;
    }
    ...
    Http.prototype = {
        ...
        ajax: function(opts) {
            ...
            var mallhost = null;
            if (opts.mallHost && _g.getLS("envir") == 'dev') {
                // mallhost = "http://192.168.10.3:60153";
                if (_g.getLS("envir")=='dev') {
                    // mallhost = 'http://183.63.145.194:60153'; //开发环境,外网访问
                    mallhost = "http://192.168.10.3:60153";
                }
                
                if (postData.mallhost) {
                    delete postData.mallhost;
                }
            }
            api && api.ajax({ 
                url: (mallhost || CONFIG.HOST) + opts.url,//'120.76.72.127:60021'+opts.url,
                method: 'post',
                timeout: opts.timeout || 60*20,
                dataType: 'json',
                returnAll: false,
                tag:opts.tag,
                data: (function(){
