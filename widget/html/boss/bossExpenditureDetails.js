define(function (require,exports,module) {
    ...
    var getTimeScope = function () {
        ...
        if(day){
            if(day.split("~").length==1){
                timeScope = day.split("~")[0] + '至' + day.split("~")[0];
            }
            if(day.split("~").length==2){
                timeScope = day.split("~")[0] + '至' + day.split("~")[1];
            }
        }
	...
	var getData = function(opts, callback, search){
        if (!search) {
            if(day){
                if(day.split("~").length==1){
    ...
    paramBringedRightSide();
     ...
     api.addEventListener && api.addEventListener({
        ...
        if(time){
            ...
            // if (!time.split("至")[0]) {
            //     day = time.split("至")[1] + '~' + time.split("至")[1];
            // } else if (!time.split("至")[1]) {
            //     day = time.split("至")[0] + '~' + time.split("至")[0];
            // } else {
            //     day = time.split("至")[0] + '~' + time.split("至")[1];
            // }
